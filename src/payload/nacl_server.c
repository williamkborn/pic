...
)
{
	...
	int one = 1;
	pic_setsockopt(sock, PIC_SOL_SOCKET, PIC_SO_REUSEADDR, &one, sizeof(one));
	...
	/* Send encrypted ACK. */
	if (encrypt_send(conn, PSK, "OK", 2) < 0) goto fail_conn;
