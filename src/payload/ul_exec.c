...
typedef struct {
	...
	Elf_Half e_type;     Elf_Half e_machine;
	Elf_Word e_version;  Elf_Addr e_entry;
	Elf_Off  e_phoff;    Elf_Off  e_shoff;
	Elf_Word e_flags;    Elf_Half e_ehsize;
	Elf_Half e_phentsize; Elf_Half e_phnum;
	Elf_Half e_shentsize; Elf_Half e_shnum;
	Elf_Half e_shstrndx;
} Elf_Ehdr;

typedef struct {
	Elf_Word p_type;   Elf_Off  p_offset;
	Elf_Addr p_vaddr;  Elf_Addr p_paddr;
	Elf_Word p_filesz; Elf_Word p_memsz;
	Elf_Word p_flags;  Elf_Word p_align;
} Elf_Phdr;
...
typedef struct {
	Elf_Word  p_type;   Elf_Word  p_flags;
	Elf_Off   p_offset; Elf_Addr  p_vaddr;
	Elf_Addr  p_paddr;  Elf_Xword p_filesz;
	Elf_Xword p_memsz;  Elf_Xword p_align;
} Elf_Phdr;
...
{
	int prot = 0;
	if (flags & PF_R) prot |= PIC_PROT_READ;
	if (flags & PF_W) prot |= PIC_PROT_WRITE;
	if (flags & PF_X) prot |= PIC_PROT_EXEC;
	return prot;
}
...
static void phase2(const struct ul_exec_config *cfg);

PIC_TEXT
__attribute__((noreturn))
static void self_remap(
	pic_uintptr blob_start,
	pic_size_t total_size,
	const struct ul_exec_config *cfg)
{
	pic_size_t alloc_size = PAGE_ALIGN_UP(total_size);

	/* Map a new RWX region at a safe high address. */
	pic_u8 *new_base = (pic_u8 *)pic_mmap(
		SAFE_ADDR_HINT, alloc_size,
	...
	PIC_LOG("ul_exec: remap %x -> %x (delta=%x, size=%x)\n",
		(long)blob_start, (long)new_base,
		(long)delta, (long)total_size);
...
PIC_TEXT
static pic_uintptr load_elf_from_memory(
	const pic_u8 *elf_data,
	pic_size_t elf_size,
	const Elf_Ehdr *ehdr,
	Elf_Addr *out_phdr_addr)
{
	...
	for (int i = 0; i < ehdr->e_phnum; i++) {
		...
		if (lo < vaddr_min) vaddr_min = lo;
		if (hi > vaddr_max) vaddr_max = hi;
	}

	pic_uintptr total = PAGE_ALIGN_UP(vaddr_max) - PAGE_ALIGN_DOWN(vaddr_min);

	if (is_pie) {
		void *region = pic_mmap(
			PIC_NULL, total, PIC_PROT_NONE,
			PIC_MAP_PRIVATE | PIC_MAP_ANONYMOUS, -1, 0);
		if ((long)region == -1) return (pic_uintptr)-1;
	...
	for (int i = 0; i < ehdr->e_phnum; i++) {
		...
		void *p = pic_mmap(
			(void *)map_start, map_size,
			PIC_PROT_READ | PIC_PROT_WRITE,
			PIC_MAP_PRIVATE | PIC_MAP_ANONYMOUS | PIC_MAP_FIXED,
			-1, 0);
		if ((long)p == -1) return (pic_uintptr)-1;
		...
		pic_mprotect((void *)map_start, map_size, pf_to_prot(phdr[i].p_flags));
	}
...
static pic_uintptr load_interp(const char *path, Elf_Addr *out_entry)
{
	...
	if (fd < 0) return (pic_uintptr)-1;

	long fsize = pic_lseek(fd, 0, PIC_SEEK_END);
	if (fsize <= 0) { pic_close(fd); return (pic_uintptr)-1; }
	pic_lseek(fd, 0, PIC_SEEK_SET);

	void *buf = pic_mmap(
		PIC_NULL, (pic_size_t)fsize,
		PIC_PROT_READ | PIC_PROT_WRITE,
		PIC_MAP_PRIVATE | PIC_MAP_ANONYMOUS, -1, 0);
	if ((long)buf == -1) { pic_close(fd); return (pic_uintptr)-1; }
...
PIC_TEXT
static pic_uintptr build_stack(
	pic_u32 argc, const char *argv_data, pic_u32 argv_size,
	pic_u32 envp_count, const char *envp_data, pic_u32 envp_size,
	Elf_Addr entry, Elf_Addr phdr_addr,
	Elf_Half phnum, Elf_Half phentsize,
	pic_uintptr interp_base)
{
	pic_size_t stack_size = 2 * 1024 * 1024;
	void *stack_base = pic_mmap(
		PIC_NULL, stack_size,
		PIC_PROT_READ | PIC_PROT_WRITE,
		PIC_MAP_PRIVATE | PIC_MAP_ANONYMOUS, -1, 0);
	if ((long)stack_base == -1) pic_exit_group(120);
	...
	for (int i = 0; i < 16; i++) rnd[i] = (pic_u8)(0x42 + i);

	/* envp strings */
	top -= envp_size;
	pic_uintptr envp_str = top;
	if (envp_size > 0) pic_memcpy((void *)envp_str, envp_data, envp_size);

	/* argv strings */
	top -= argv_size;
	pic_uintptr argv_str = top;
	if (argv_size > 0) pic_memcpy((void *)argv_str, argv_data, argv_size);
	...
	*sp++ = 0;
	...
	/* auxv */
#define AUX(t, v) do { *sp++ = (pic_uintptr)(t); *sp++ = (pic_uintptr)(v); } while (0)
...
PIC_TEXT
__attribute__((noreturn))
static void jump_to_entry(pic_uintptr entry, pic_uintptr sp)
{
...
static void phase2(const struct ul_exec_config *cfg)
{
	...
	PIC_LOG("ul_exec: elf_size=%d type=%d entry=%x phnum=%d\n",
		(long)cfg->elf_size, (long)ehdr->e_type,
		(long)ehdr->e_entry, (long)ehdr->e_phnum);
	...
	for (int i = 0; i < ehdr->e_phnum; i++) {
		if (phdr[i].p_type == PT_INTERP) {
			interp_path = (const char *)(elf_data + phdr[i].p_offset);
			break;
		}
	}
	...
	pic_uintptr elf_base = load_elf_from_memory(
		elf_data, cfg->elf_size, ehdr, &phdr_addr);
	if (elf_base == (pic_uintptr)-1) pic_exit_group(103);

	Elf_Addr entry = elf_base + ehdr->e_entry;
	PIC_LOG("ul_exec: loaded base=%x entry=%x\n",
		(long)elf_base, (long)entry);
	...
	if (interp_path) {
		...
		if (interp_base == (pic_uintptr)-1) pic_exit_group(106);
		interp_entry = interp_base + ie_entry;
		PIC_LOG("ul_exec: interp base=%x entry=%x\n",
			(long)interp_base, (long)interp_entry);
	}

	/* Build stack. */
	pic_uintptr sp = build_stack(
		cfg->argc, argv_data, cfg->argv_size,
		cfg->envp_count, envp_data, cfg->envp_size,
		entry, phdr_addr, ehdr->e_phnum, ehdr->e_phentsize,
		interp_base);
...
void _start(void)
{
	...
	pic_size_t total_size = (cfg_start - blob_base) +
		sizeof(struct ul_exec_config) +
		cfg->elf_size + cfg->argv_size + cfg->envp_size;
