...
Creates a venv, installs `picblobs[dev]` via [uv](https://github.com/astral-sh/uv),
and activates it. Idempotent — safe to re-source.
...
### Build the full product for a platform
...
`//release:full` includes all blobs and test runners. Platform and build mode
are orthogonal — combine any platform config with `--config=debug` or
`--config=release`.
...
```bash
for cfg in linux_x86_64 linux_i686 linux_aarch64 linux_armv5_arm \
           linux_armv5_thumb linux_s390x linux_mipsel32 linux_mipsbe32 \
  ...
  bazel build //release:full --config="$cfg"
done
```
...
Platform configs (defined in `.bazelrc`, generated by `tools/generate.py`):
...
Build mode configs (orthogonal to platform):
...
8. Build, stage, verify:
   ```bash
   bazel build //release:full --config=linux_{arch}
