...
This runs `tools/stage_blobs.py`, which builds all supported blobs and
runners for Linux, FreeBSD, and Windows, plus the Linux `ul_exec` verifier
fixtures. It then stages build outputs into the package trees and regenerates
the runtime sidecar catalog:
